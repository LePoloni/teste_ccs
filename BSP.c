 ...
 *	Revisão: 1 (17/03/2019)
 *	Tornei a função writedata pública para usar com o SD Card
 *	Revisão: 2 (19/03/2019)
 *	Implementadas funções para desenho de retângulo e circulo
 *	Revisão: 3 (31/03/2019)
 *	Correção na função para desenho de circulo
 *	Implementadas funções para desenho de circulo preenchido
 *	Revisão: 4 (01/04/2019)
 *	Alterei o clock do LCD de 4 MHz para 8 MHz
 *	Correções nas funções para desenho de circulos
 *  Revisão: 5 (04/05/2019)
 *  Alterei a função de incremento de ponto na plotagem do LCD para retornar o próximo ponto
 *	Revisão: 6 (11/05/2019)
 *	Implementada a função BSP_LCD_PlotLine para conectar pontos em gráficos
 *	Implementada a função BSP_LCD_DrawString2 para escrever strings com cor de fundo selecionável
 *	Revisão: 7 (01/09/2019)
 *	Tornei a função setAddrWindow pública para usar com o SD Card
 *	Revisão: 8 (11/09/2019)
 *	Criei defines para uso de funções (incialmente internas: writedata e setAddrWindow) o formato padrão da biblioteca
 *	Revisão: 9 (29/09/2019)
 *	Criei uma função para desenhar linhas diagonais BSP_LCD_DrawDiagonalLine
 */
...
//        color 16-bit color, which can be produced by BSP_LCD_Color565()
// Output: none
void BSP_LCD_DrawDiagonalLine(int16_t x, int16_t y, int16_t w, int16_t h, uint16_t color) {
  int32_t delta, i, modw, modh;
	
	//Calcula os módulos
	modw = w<0?-w:w;
	modh = h<0?-h:h;
	
	//Define h em função de w: h = f(w) = aw + b
	if(modw>modh)
	{
		//Cálculo do coeficiente angular * 200
		delta = ((h-1)*200)/w;
		...
		if(w>0)
		{
			...
			{
				//Ajuste de x e y
				BSP_LCD_DrawPixel(x+i, y+delta*i/200, color);
			}
		}
		else
		{
	...
	//Senão define w em função de h: w = f(h) = ah + b
	else
	{
		//Cálculo do coeficiente angular * 200
		delta = ((w-1)*200)/h;
		...
		if(h>0)
		{
			...
			{
				//Ajuste de x e y
				BSP_LCD_DrawPixel(x+delta*i/200, y+i, color);
			}
		}
		else
		{
...
// bgColor is Black and size is 1
// Output: number of characters printed
uint32_t BSP_LCD_DrawString2(uint16_t x, uint16_t y, char *pt, int16_t textColor, int16_t bgColor){
  uint32_t count = 0;
  if(y>12) return 0;
  while(*pt){
...
//Leandro (11/05/2019)
// ------------BSP_LCD_PlotLine------------
// Plot a line on the chart.  To plot several points in the
// same column, call this function repeatedly before calling
// BSP_LCD_PlotIncrement().  The units of the data are the
// same as the ymax and ymin values specified in the
// initialization function.
// Input: data1  start point of the line to be plotted (units not specified)
//        data2	 end point of the line to be plotted (units not specified)
//        color1 16-bit color for the point, which can be produced by BSP_LCD_Color565()
// Output: none
// Assumes: BSP_LCD_Init() and BSP_LCD_Drawaxes() have been called
void BSP_LCD_PlotLine(int32_t data1, int32_t data2, uint16_t color1){
  data1 = ((data1 - Ymin)*100)/Yrange;
  if(data1 > 98){
    data1 = 98;
    color1 = LCD_RED;
  ...
  BSP_LCD_DrawPixel(TimeIndex + 11, 115 - data1, color1);
	if(data1 >= data2)
	{
		while(((116 - data1) + (data1-data2+1))>120);
		BSP_LCD_DrawFastVLine(TimeIndex + 11,116 - data1, data1-data2+1, color1);
	}
	else
	{
		while(((116 - data2) + (data2-data1+1))>120);
		BSP_LCD_DrawFastVLine(TimeIndex + 11,115 - data2, data2-data1+1, color1);
	}
}
...
// Input: none
// Output: next point on x-axis
// Assumes: BSP_LCD_Init() and BSP_LCD_Drawaxes() have been called
int BSP_LCD_PlotIncrement(void){
